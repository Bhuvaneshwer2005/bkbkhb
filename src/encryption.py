...
def save_key(key: Dict[str, float], filepath: str, round_states: List[float] = None, substitution_data: List[np.ndarray] = None) -> None:
    """
    Save TTEH key to file with all decryption data.
    
    Args:
        key: Key dictionary from generate_key()
        filepath: Path to save key file
        round_states: List of map states from encryption (for decryption)
        substitution_data: List of substitution data from encryption (for decryption)
    """
    import json
    import numpy as np
    ...
    # Add decryption data if provided
    if round_states is not None:
        key_data['round_states'] = round_states
    if substitution_data is not None:
        # Convert numpy arrays to lists for JSON serialization
        key_data['substitution_data'] = [arr.tolist() for arr in substitution_data]
...
def load_key(filepath: str) -> Dict:
    """
    Load TTEH key from file with all decryption data.
    
    Args:
        filepath: Path to key file
    
    Returns:
        Dictionary containing x0, mu, and optionally round_states and substitution_data
    """
    import json
    import numpy as np
    try:
        ...
        # Convert substitution_data back to numpy arrays if present
        if 'substitution_data' in key_data:
            key_data['substitution_data'] = [np.array(arr, dtype=np.uint8) for arr in key_data['substitution_data']]
